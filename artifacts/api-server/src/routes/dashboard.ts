...
router.get("/dashboard/summary", async (_req, res): Promise<void> => {
  ...
  const [
    ...
    db.select({ count: count() }).from(alertsTable).where(eq(alertsTable.status, "firing")),
    db.select({ count: count() }).from(alertsTable).where(
      and(eq(alertsTable.severity, "critical"), eq(alertsTable.status, "firing"))
    ),
    db.select({ count: count() }).from(logsTable).where(gte(logsTable.timestamp, since1min)),
    db.select().from(slosTable),
    db.select().from(apmMetricsTable).where(gte(apmMetricsTable.timestamp, since5min)).orderBy(desc(apmMetricsTable.timestamp)).limit(20),
    db.select({ count: count() }).from(slosTable).where(
      and(eq(slosTable.status, "at_risk"))
    ),
  ]);
...
router.get("/dashboard/traffic", async (_req, res): Promise<void> => {
  ...
  for (const m of metrics) {
    ...
    if (!buckets.has(key)) buckets.set(key, { requests: 0, errors: 0, responseTimes: [] });
  ...
  if (buckets.size < 5) {
    ...
    for (let i = 59; i >= 0; i--) {
      ...
      points.push({
        ...
        requests,
        errors,
        responseTime: parseFloat((isPeak ? 200 + Math.random() * 300 : 50 + Math.random() * 100).toFixed(1)),
      });
    }
  ...
  for (let i = 59; i >= 0; i--) {
    ...
    points.push({
      time: key,
      requests: b ? b.requests : Math.floor(Math.random() * (isPeak ? 200 : 30)),
      errors: b ? b.errors : Math.floor(Math.random() * 5),
      responseTime: b && b.responseTimes.length > 0
        ? parseFloat((b.responseTimes.reduce((a, c) => a + c, 0) / b.responseTimes.length).toFixed(1))
        : parseFloat((isPeak ? 150 + Math.random() * 200 : 40 + Math.random() * 60).toFixed(1)),
    });
  }
