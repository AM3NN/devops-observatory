...
import { db, logsTable, apmMetricsTable, tracesTable, ingestionAgentsTable } from "@workspace/db";
...
async function requireApiKey(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  const [agent] = await db.select().from(ingestionAgentsTable).where(eq(ingestionAgentsTable.apiKey, key));
...
function validateLogEntry(entry: any): string | null {
  if (!entry || typeof entry !== "object") return "Each log entry must be an object";
  if (!entry.service || typeof entry.service !== "string") return "log.service is required";
  if (!entry.message || typeof entry.message !== "string") return "log.message is required";
  if (entry.level && !VALID_LEVELS.has(entry.level)) return `log.level must be one of: ${[...VALID_LEVELS].join(", ")}`;
  return null;
}

function validateMetric(body: any): string | null {
  if (!body || typeof body !== "object") return "Body must be an object";
  if (!body.service || typeof body.service !== "string") return "service is required";
  if (typeof body.responseTime !== "number") return "responseTime must be a number";
  ...
  if (typeof body.memoryUsage !== "number") return "memoryUsage must be a number";
  if (typeof body.activeConnections !== "number") return "activeConnections must be a number";
  return null;
}

function validateSpan(span: any): string | null {
  if (!span || typeof span !== "object") return "Each span must be an object";
  if (!span.traceId || typeof span.traceId !== "string") return "span.traceId is required";
  if (!span.spanId || typeof span.spanId !== "string") return "span.spanId is required";
  if (!span.service || typeof span.service !== "string") return "span.service is required";
  if (!span.operation || typeof span.operation !== "string") return "span.operation is required";
  if (typeof span.duration !== "number") return "span.duration must be a number";
  if (span.status && !VALID_STATUSES.has(span.status)) return "span.status must be ok|error|timeout";
  return null;
}
...
// POST /ingest/logs
router.post("/ingest/logs", requireApiKey, async (req: Request, res: Response): Promise<void> => {
  const body = req.body;
  if (!body || !Array.isArray(body.logs) || body.logs.length === 0) {
    res.status(400).json({ error: "Body must have a non-empty 'logs' array" });
    return;
  }
  ...
  const rows = body.logs.map((entry: any) => ({
    ...
    metadata: entry.metadata ? { ...entry.metadata, _agent: agentId } : { _agent: agentId },
  }));
  ...
  if (agentId !== "demo-agent") {
    await db
      ...
      .set({ lastSeen: new Date(), totalLogs: sql`total_logs + ${rows.length}` })
      .where(eq(ingestionAgentsTable.id, agentId));
  }

  res.status(201).json({ ingested: rows.length, ids: rows.map((r: any) => r.id) });
});

// POST /ingest/metrics
router.post("/ingest/metrics", requireApiKey, async (req: Request, res: Response): Promise<void> => {
  const body = req.body;
...
// POST /ingest/traces
router.post("/ingest/traces", requireApiKey, async (req: Request, res: Response): Promise<void> => {
  const body = req.body;
  if (!body || !Array.isArray(body.spans) || body.spans.length === 0) {
    res.status(400).json({ error: "Body must have a non-empty 'spans' array" });
    return;
  }
  ...
  if (agentId !== "demo-agent") {
    await db
      .update(ingestionAgentsTable)
      .set({ lastSeen: new Date(), totalTraces: sql`total_traces + ${rows.length}` })
      .where(eq(ingestionAgentsTable.id, agentId));
  }
...
router.post("/ingest/heartbeat", requireApiKey, async (req: Request, res: Response): Promise<void> => {
  const agentId = (req as any).agentId as string;
  if (agentId !== "demo-agent") {
    await db
      .update(ingestionAgentsTable)
...
// GET /ingest/agents
router.get("/ingest/agents", async (_req: Request, res: Response): Promise<void> => {
  const agents = await db.select().from(ingestionAgentsTable).orderBy(ingestionAgentsTable.createdAt);

  // Always include the demo agent entry
  const demoAgent = {
    id: "demo-agent",
    name: "Demo Agent (clé globale)",
    description: "Clé de démonstration — utiliser pour les tests rapides",
    apiKey: DEMO_KEY,
    status: "active",
    host: null,
    language: null,
  ...
  res.json([demoAgent, ...agents.map((a) => ({ ...a, lastSeen: a.lastSeen?.toISOString() ?? null, createdAt: a.createdAt.toISOString() }))]);
});

// POST /ingest/agents — register a new agent and get a unique API key
router.post("/ingest/agents", async (req: Request, res: Response): Promise<void> => {
  const body = req.body;
  ...
  logger.info({ agentId: id, name: body.name }, "New ingestion agent registered");

  res.status(201).json({ ...agent, lastSeen: agent.lastSeen?.toISOString() ?? null, createdAt: agent.createdAt.toISOString() });
});
