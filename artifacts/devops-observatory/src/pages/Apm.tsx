...
export default function Apm() {
  ...
  const {
    ...
    selectedService !== "all"
      ? { service: selectedService, limit: 10 }
      : { limit: 10 },
  );
  const serviceList = Array.isArray(services) ? services : [];
  const metricSeries = Array.isArray(metrics) ? metrics : [];
  const traceList = Array.isArray(traces) ? traces : [];

  if (isServicesError || isMetricsError || isTracesError) {
    return (
      <PageState
        className="max-w-7xl"
        icon={Zap}
        title="APM data is unavailable"
        description="Metrics, traces, or service inventory could not be loaded."
        actionLabel="Retry"
        onAction={() => {
        ...
        >
          ...
          <div className="h-[250px] w-full">
            {loadingMetrics ? (
              ...
              <PageState
                icon={Network}
                title="No latency metrics"
                description="No latency data is available for the selected service yet."
              />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  data={metricSeries}
                  margin={{ left: -20, right: 10, bottom: 0, top: 10 }}
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
          ...
          <div className="h-[250px] w-full">
            {loadingMetrics ? (
              ...
              <PageState
                ...
                description="CPU and memory data is not available for the current selection."
              />
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead className="bg-background/50">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold text-slate-400 text-right">
                  Duration (ms)
                </th>
                <th className="px-6 py-3 font-semibold text-slate-400 text-center">
                  Status
                </th>
              </tr>
            </thead>
                ...
                traceList.map((trace) => (
                  ...
                  >
                      ...
                      <div className="flex items-center justify-end">
                        <div className="w-24 bg-slate-800 h-1.5 rounded-full overflow-hidden mr-3">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (trace.duration / 2000) * 100)}%`,
                            }}
                          />
                        </div>
                        <span
                    ...
                    <td className="px-6 py-3 text-center">
                      <div
                        className={`inline-block w-2 h-2 rounded-full ${trace.status === "ok" ? "bg-success" : trace.status === "timeout" ? "bg-warning" : "bg-destructive shadow-[0_0_8px_rgba(248,113,113,0.8)] animate-pulse"}`}
                      />
                    </td>
                  </tr>
                ))
              )}
              {!loadingTraces && traceList.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-8">
                    <PageState
                      icon={Database}
                      title="No traces found"
                      description="No distributed traces are available for the current filter."
                    />
                  </td>
                </tr>
              )}
