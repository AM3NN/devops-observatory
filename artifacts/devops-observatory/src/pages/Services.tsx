...
export default function Services() {
  const { data: services, isLoading } = useServicesPoll();
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-400">Inventory and health status of all registered microservices</p>
      </div>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${
                    service.environment === 'production' ? 'bg-primary/10 text-primary border-primary/20' : 
                    service.environment === 'staging' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
              ...
              <div className={`flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${
                service.status === 'healthy' ? 'bg-success/10 text-success border-success/20' :
                service.status === 'degraded' ? 'bg-warning/10 text-warning border-warning/20' :
                'bg-destructive/10 text-destructive border-destructive/20 animate-pulse'
              }`}>
                <div className={`w-2 h-2 rounded-full mr-1.5 ${
                  service.status === 'healthy' ? 'bg-success' : service.status === 'degraded' ? 'bg-warning' : 'bg-destructive'
                }`} />
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-background/50 rounded-xl p-3 border border-white/5">
                ...
                <p className={`text-lg font-mono font-bold ${service.uptime < 99.9 ? 'text-warning' : 'text-slate-200'}`}>
                  {service.uptime}%
                </p>
              </div>
              <div className="bg-background/50 rounded-xl p-3 border border-white/5">
              ...
              <div className="bg-background/50 rounded-xl p-3 border border-white/5">
                ...
                <p className="text-lg font-mono font-bold text-slate-200">
                  {service.requestsPerMin} <span className="text-xs text-slate-500 font-sans font-normal">req/m</span>
                </p>
              </div>
              <div className="bg-background/50 rounded-xl p-3 border border-white/5">
                <p className="text-[10px] text-slate-500 font-mono uppercase mb-1 flex items-center">
                ...
                <p className={`text-lg font-mono font-bold ${service.errorRate > 1 ? 'text-destructive' : 'text-slate-200'}`}>
                  {service.errorRate}%
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-white/5 text-xs text-slate-500 flex justify-between items-center">
              ...
              <span className="font-medium text-slate-300 bg-white/5 px-2 py-1 rounded">{service.team}</span>
            </div>
          </motion.div>
        ))}
      </div>
